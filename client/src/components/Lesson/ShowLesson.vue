    ...
    v-row(no-gutters)
      ...
      v-col.p-0(cols='3')
        ...
        v-card.elevation-12.pa-0(:height="height")
          v-toolbar(light v-if="this.lesson")
            v-toolbar-title.text-h4 {{ this.lesson.name }}
            v-spacer
          v-card-text.py-0
            v-list
              v-list-item(v-for="rhythm in rhythms" :key="rhythm.title" @click="display_rhythm($event, rhythm)" active @dblclick="edit_rhythm_title")
                v-list-item-icon
                ...
                v-list-item-content
                  ...
                  v-text-field#title_edit(v-model="active_rhythm_title" type="text" @input="enable_save_btn" v-else)
                v-spacer
              v-list-item
...
export default {
  ...
  computed: {
    lesson () {
      return this.$store.getters.getLesson(this.$route.params.id)
    },
    
    rhythms () {
      return this.lesson ? this.lesson.rhythms : null
    },

    ...mapState(['user', 'students', 'tutors', 'lessons'])
  },
  watch: {
    bpm: function (val) {
  ...
  methods: {
    ...
    update_rhythms () {
      ...
      this.$store.dispatch('editRhythm', {
        newRhythm: newRhythm,
        oldRhythm: this.active_rhythm,
        lesson: this.lesson
      })
